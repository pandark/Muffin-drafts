...
* `=` pour réindenter automatiquement (par exemple `gg=G` aller à la première ligne et réindenter jusqu'à la dernière). Il y a deux cas pour lesquels l'indentation par défaut de Vim et celle de la norme ne sont pas en accord : les commentaires (vim met un espace puis une astérisques alors que la norme veut deux astérisques sans espace avant), et les conditions sur plusieurs lignes pour les structures de contrôle (vim rajoute deux tabs alors que la norme n'en veut qu'un).
